...
const int IDLE_TIMEOUT_MS = 5;
const double POLL_INTERVAL_SEC = 0.01;

// DPI 调整参数
const int DRAG_DPI_SPEED = 2;  // 拖动时的低速 DPI 设置 (1-20)
...
// DPI 控制器类 - 专门用于拖动时的 DPI 调整
class DPIController {
private:
    int originalSpeed;
    bool isDragMode;
    std::chrono::steady_clock::time_point lastChangeTime;
    static const int CHANGE_DEBOUNCE_MS = 5;  // 5ms 防抖

public:
    DPIController() : originalSpeed(10), isDragMode(false) {
        originalSpeed = getCurrentDPI();
    ...
    void enterDragMode() {
        if (!isDragMode) {
            isDragMode = true;
            if (setDPI(DRAG_DPI_SPEED)) {
                std::wcout << L"DPI 降低到拖动模式 (" << DRAG_DPI_SPEED << L"/20)" << std::endl << std::flush;
            }
        }
    }
    ...
    void exitDragMode() {
        if (isDragMode) {
            isDragMode = false;
            if (setDPI(originalSpeed)) {
                std::wcout << L"DPI 恢复正常 (" << originalSpeed << L"/20)" << std::endl << std::flush;
            }
        }
    }

    void restoreOriginalDPI() {
        if (isDragMode) {
            exitDragMode();
        }
    }
...
public:
    ...
    void showSettingsInterface() {
        ...
        std::wcout << L"    双鼠标拖拽助手 v2.0 - 设置界面" << std::endl << std::flush;
        ...
        std::wcout << L"原始速度: " << originalSpeed << L"/20" << std::endl << std::flush;
        std::wcout << L"拖动时 DPI: " << DRAG_DPI_SPEED << L"/20 (自动切换)" << std::endl << std::endl << std::flush;
        ...
        std::wcout << L"新功能: 拖动时自动降低到 " << DRAG_DPI_SPEED << L" 级别" << std::endl << std::flush;
    }
...
private:
    ...
    DPIController* dpiController;  // DPI 控制器指针

public:
    DragHelper() : running(false), enabled(false), hwnd(nullptr), atom(0),
                   currentState(NORMAL), dpiController(nullptr) {
    ...
    void setDPIController(DPIController* controller) {
    ...
    LRESULT wndProc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
        if (msg == WM_INPUT && enabled) {
            ...
            if (currentState == NORMAL && isLeft) {
                ...
                // 降低 DPI
                if (dpiController) {
                    dpiController->enterDragMode();
                }
                
                std::wcout << L"左鼠标移动，开始拖拽 (DPI已降低)" << std::endl << std::flush;
                return 0;
            }
                ...
                if (isLeft) {
                    ...
                    std::wcout << L"右鼠标移动，结束拖拽 (DPI已恢复)" << std::endl << std::flush;
                    return 0;
                }
            }
        }
    ...
    bool enable() {
        ...
        try {
            ...
            enabled = true;
            running = true;
            std::wcout << L"双鼠标拖拽助手已启用 (增强 DPI 版本)" << std::endl << std::flush;
            return true;
    ...
    void disable() {
        ...
        // 恢复 DPI
        if (dpiController) {
            dpiController->restoreOriginalDPI();
        }
...
DPIController* g_pDPIController = nullptr;
...
BOOL WINAPI ConsoleHandler(DWORD CEvent)
{
    ...
    case CTRL_C_EVENT:
        ...
        if(g_pDPIController) {
            g_pDPIController->restoreOriginalDPI();
        }
        break;
    }
    return TRUE;
...
int main() {
    ...
    // 设置控制台标题
    SetConsoleTitleW(L"双鼠标拖拽助手 v2.0 - DPI 增强版");

    std::wcout << L"启动程序..." << std::endl << std::flush;

    std::wcout << L"=== 双鼠标拖拽助手 v2.0 - DPI 增强版 ===" << std::endl << std::flush;
    ...
    std::wcout << L" --- 新功能：拖动时自动降低 DPI，释放时立即恢复 ---" << std::endl << std::flush;
